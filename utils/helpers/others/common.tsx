...
import { ROLES } from '@/utils/constants/settings/workspace'
...
export const getVariablesFromString = (
  subject = '',
  body = '',
...
export const mergeUniqueArray = <T extends Record<string, any>>(
  existingArray: T[],
  newArray: T[],
  key = 'id',
): T[] => {
...
export const mapUniqueObjectsInArray = <T extends Record<string, any>>(
  arr: T[],
  prop: string,
): T[] => {
...
export const getSortOrder = (order: number): 'ASC' | 'DESC' =>
  order === 1 ? 'DESC' : 'ASC'
...
export const setAndGetSortOrder = (
  ...
  const newOrder =
    sortField.fieldName === keyword ? (sortField.order === 0 ? 1 : 0) : 1
...
}): boolean => user?.role === ROLES.OWNER && user?.workspaceId === null

export const checkIfSuperUser = (): boolean =>
  Cookies.get(COOKIES.SUPER_USER) === 'true'

export const validateGoogleSheetsUrl = (url: string): boolean =>
  /https:\/\/docs\.google\.com\/spreadsheets\/d\/([a-zA-Z0-9-_]+)\/edit/.test(
    url,
  )

export const getSpreadsheetIdAndSheetName = (
  link: string,
  ...
  return {
    spreadsheetId: spreadsheetIdMatch?.[1] || null,
    sheetName: sheetNameMatch ? decodeURIComponent(sheetNameMatch[1]) : null,
  }
}

export const generateGoogleSheetsUrl = (
...
export const mapUniqueObjectsByProp = <T extends Record<string, any>>(
  arr: T[],
  prop: string,
): T[] => {
...
export const getStatusColor = (
  status?: string,
  type?: 'bg' | 'text',
  customAiLabels?: { name: string; interest_status?: string }[],
): string => {
  ...
  switch (normalizedStatus) {
    case 'lead':
      return type === 'bg' ? 'bg-white' : ''
    case 'interested':
      return type === 'bg' ? 'bg-[#84E662]' : '#84E662'
    case 'meeting booked':
      return type === 'bg' ? 'bg-[#9C62E6]' : '#9C62E6'
    case 'meeting completed':
      return type === 'bg' ? 'bg-[#E6D162]' : '#E6D162'
    case 'closed':
      return type === 'bg' ? 'bg-[#626FE6]' : '#626FE6'
    case 'out of office':
      return type === 'bg' ? 'bg-[#62B6E6]' : '#62B6E6'
    case 'wrong person':
      return type === 'bg' ? 'bg-[#627FE6]' : '#627FE6'
    case 'not interested':
      return type === 'bg' ? 'bg-[#E662A1]' : '#E662A1'
    default:
      switch (customLabel?.interest_status) {
        case 'positive':
          return type === 'bg' ? 'bg-green-600' : '#16a34a'
        case 'negative':
          return type === 'bg' ? 'bg-red-600' : '#dc2626'
        case 'neutral':
          return type === 'bg' ? 'bg-yellow-600' : '#ca8a04'
        default:
          return type === 'bg' ? 'border-gray-700' : ''
      }
  }
}
...
export const cleanHtmlEmail = (html = ''): string => {
  ...
  const cleanElements = (node: Element | ChildNode) => {
    if (!('children' in node) || !node.children) return
    node.children = (node.children as ChildNode[]).flatMap((child) => {
      if (
        child.type === 'tag' &&
        (child as Element).attribs?.id === 'watermark-reachinbox'
      )
        return []
      if (
        child.type === 'tag' &&
        (child as Element).name === 'span' &&
        (child as Element).attribs?.class
          ?.split(' ')
          .includes('highlighted-spam-word')
      )
        return (child as Element).children || []
      if (child.type === 'tag' && (child as Element).name === 'br') {
        if ((child as Element).attribs?.style)
          delete (child as Element).attribs.style
        return [child]
      }
      if (child.type === 'tag' && (child as Element).name === 'p') {
        ;(child as Element).name = 'div'
        ;(child as Element).attribs = {}
        return [child]
      }
      if ('data' in child && child.data)
        child.data = child.data.replace(/&#xa0;/g, '&nbsp;')
      if (child.type === 'tag' && (child as Element).name === 'div') {
        const hasMeaningfulChildren = (child as Element).children?.some(
          (c) =>
            c.type !== 'text' ||
            ('data' in c && c.data && c.data.trim() !== ''),
        )
        if (!hasMeaningfulChildren)
          return [
            { type: 'tag', name: 'br', children: [] } as unknown as ChildNode,
          ]
        return [child]
      }
      if (
        child.type === 'tag' &&
        ['table', 'td', 'tr'].includes((child as Element).name)
      )
        return [child]
      if ('data' in child && child.data)
  ...
  cleanElements(document as unknown as Element)
  text = DomUtils.getOuterHTML(document)
  text = decode(text)
  text = text.replace(
    /<span[^>]*class=["']?highlighted-spam-word["']?[^>]*>([\s\S]*?)<\/span>/g,
    '$1',
  )
...
export const checkWorkspaceIdInParams = (
  url: string,
): { workspaceId: string | null; initialUrl: string } => {
  try {
...
export const checkForMainTabs = (pathname: string): boolean => {
  const MAIN_PATH = [
    ...
    PATH.COUPON,
    PATH.BUY_DOMAIN,
    PATH.WEBSITE_VISITOR,
    PATH.INBOX_PLACEMENT,
  ]
...
export const getAddonPurchaseTitleAndDescription = (addons?: string[]) => {
  if (!addons || addons.length > 1)
    return {
      ...
      description:
        'Upgrade your account to access these premium features and enhance your workflow.',
    }
  switch (addons[0]) {
    case 'leads':
      return {
        title: 'Expand Your Lead Generation',
        description:
          'Purchase lead credits to grow your prospect database and reach more potential customers.',
      }
    case 'email':
      return {
        title: 'Boost Your Email Outreach',
        description:
          'Add more email credits to scale your campaigns and connect with more prospects.',
      }
    case 'ai':
      return {
        title: 'Power Up with AI Writing',
        description:
          'Get more AI word credits to create compelling messages and content that converts.',
      }
    case 'workspace':
      return {
        title: 'Add Team Workspaces',
        description:
          "Expand your team's collaboration capabilities with additional workspace licenses.",
      }
    case 'emailVerifier':
      return {
        title: 'Verify Email Addresses',
        description:
          'Improve deliverability and engagement with our email verification service.',
      }
    default:
      return {
        title: 'Unlock Premium Features',
...
export const getEmailDepletedBannerTitleAndDescription = (
  userHasUnlimitedEmail?: boolean,
  userHasUnlimitedLeads?: boolean,
) => {
  if (!userHasUnlimitedEmail && !userHasUnlimitedLeads)
    return {
      title: 'Purchase Additional Email & Lead Credits',
      description:
        'Top up your Email and Active Lead credits to keep your campaigns running smoothly without interruptions.',
    }
  if (!userHasUnlimitedEmail)
    return {
      title: 'Purchase Additional Email Credits',
      description:
        'Add more Email credits to maximize your outreach and send more campaigns with ease.',
    }
  return {
    title: 'Unlock Premium Features',
    description:
      'Upgrade your account to access premium features and enhance your workflow.',
  }
}
